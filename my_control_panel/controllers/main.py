...
class MyControlPanel(http.Controller):

    @http.route('/my_control_panel/data', type='json', auth='user')
    def my_control_panel_data(self, **kw):
        if not request.env.user.has_group('my_control_panel.group_saas_manager'):
    ...
    @http.route('/web/underdevelop', type='http', auth='user')
    def underdevelop(self):
        return "Sorry ..This area is underdevelopment"